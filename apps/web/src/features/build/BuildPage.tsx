...
export function BuildPage() {
  const [params, setParams] = useSearchParams(); const [profiles, setProfiles] = useState<ModelProfile[]>([]); const [created, setCreated] = useState<ProjectCreated>(); const [report, setReport] = useState<Report>(); const [error, setError] = useState(''); const { projectId, projects, refreshProjects } = useProject()
  ...
  return <section className="page build-page"><header className="page-header"><div><p className="eyebrow">ONLINE BUILD · 06</p><h1>从文本，生长出图谱</h1><p>上传一部小说，观察章节、实体、事实与证据如何逐步进入知识图谱。</p></div></header>{error && <p role="alert" className="error-state">{error}</p>}<div className="build-grid"><div className="build-source-stack"><UploadStep profiles={profiles} onCreated={onCreated} /><AttributeBackfill project={currentProject} profiles={profiles} onCreated={onAttributeJob} /></div>{created ? <JobProgress initial={created.job} onChange={onJob} /> : <div className="build-placeholder"><span>二</span><p>任务建立后，这里会显示可恢复的抽取进度。</p></div>}{created && report ? <QualityReport report={report} projectId={created.project.id} /> : <div className="build-placeholder"><span>三</span><p>构建结束后展示实体、事实、证据与拒绝原因。</p></div>}</div></section>
}
